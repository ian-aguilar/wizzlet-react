...
const AdminDashboard = () => {
  ...
  //-------------Custom Hooks-----------------------
  const { getAdminDashboardUserDataDataApi, isLoading: usersDetailsLoading } =
    useGetAdminDashboardUserDataDataApi();

  const { getAdminDashboardCategoryDataApi, isLoading: categoryLoading } =
    useGetAdminDashboardCategoryDataApi();

  const { getMarketplaceListingAPI } = useMarketplaceListingAPI();

  const { getDashboardRevenueDataApi, isLoading: revenueLoading } =
    useGetDashboardRevenueDataApi();
  ...
  return (
    <>
      <section className="AdminDashboard h-full w-full bg-white overflow-y-auto scroll-design p-5 ">
        ...
        {revenueLoading || categoryLoading || usersDetailsLoading ? (
          <Loader />
        ) : (
          <>
